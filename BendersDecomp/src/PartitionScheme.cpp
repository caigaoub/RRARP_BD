...
void PartitionScheme::build(DataHandler& dataset, int nb_dstzn, int type_trajc) {
	...
	_real_G.resize(_size_G);
	for (int i = 0; i < _size_G; i++) {
		_real_G[i].resize(_size_G, make_pair(false, 0));
	}
...
pair<TOUR, double> PartitionScheme::solve_shortestpath_path(vector<int> & seq) {
	int idxmat_1, idxmat_2, idxmat_3;
	vector<double> entry_dist(_nb_dstzn, INF);
	vector<double> exit_dist(_nb_dstzn, INF);
	vector<double> INF_dist(_nb_dstzn, INF);
	...
	for (int i = 0; i < _nb_dstzn; i++)	{
		...
		if (_real_G[0][idxmat_1 + i].first == true) {
			entry_dist[i] = _real_G[0][idxmat_1 + i].second;
			SDS[1][i] = entry_dist[i];
	...
	for (int i = 0; i < _nb_dstzn; i++) {
		for (int j = 0; j < _nb_dstzn; j++) {
			if (_real_G[idxmat_1 + i][idxmat_2 + j].first == true) {
				dist = entry_dist[i] + _real_G[idxmat_1 + i][idxmat_2 + j].second;
				if (dist < exit_dist[j]){
					exit_dist[j] = dist;
	...
	for(pos = 2; pos <= _dataset->_nb_targets; pos++) {
		...
		for (int i = 0; i < _nb_dstzn; i++) {
			for (int j = 0; j < _nb_dstzn; j++) {
				if (_real_G[idxmat_1 + i][idxmat_2 + j].first == true) {
					dist = exit_dist[i] + _real_G[idxmat_1 + i][idxmat_2 + j].second;
					if (dist < entry_dist[j]){
						entry_dist[j] = dist;
		...
		for (int i = 0; i < _nb_dstzn; i++) {
			for (int j = 0; j < _nb_dstzn; j++) {
				if (_real_G[idxmat_2 + i][idxmat_3 + j].first == true) {
					dist = entry_dist[i] + _real_G[idxmat_2 + i][idxmat_3 + j].second;
					if (dist < exit_dist[j]){
						exit_dist[j] = dist;
	...
	SDS[_dataset->_nb_targets + 1].resize(1);
	idxmat_1 = (idx_circle - 1) * 2 * _nb_dstzn + _nb_dstzn + 1;
	// cout << idx_circle << ", " << _size_G << endl;
	for (int i = 0; i < _nb_dstzn; i++) {
		if (_real_G[idxmat_1 + i][_size_G - 1].first == true) {
			dist = exit_dist[i] + _real_G[idxmat_1 + i][_size_G - 1].second;
			if (dist < dist_endDepot){
				dist_endDepot = dist;
	...
	BdyPoint bp = {_dataset->_nb_targets+1, 0};
	OptPath.push_front(make_pair(bp, _points[_dataset->_nb_targets+1][0]));
	OptPath.push_front(make_pair(prevnode_arrival, _points[prevnode_arrival._tarID][prevnode_arrival._subID]));
	// int pre_tar = prevnode_arrival.first;
	int tpidx_pre_tar = prevnode_arrival._subID;
	for(int i = 2*_dataset->_nb_targets-1; i >= 0; i--){
		OptPath.push_front(make_pair(prevnodes_tars[i][tpidx_pre_tar], _points[prevnodes_tars[i][tpidx_pre_tar]._tarID][prevnodes_tars[i][tpidx_pre_tar]._subID]));
	...
	bp = {_dataset->_nb_targets+1,0};
	tour_star[_dataset->_nb_targets+1] = make_pair(bp, bp);

	
	return make_pair(tour_star, SDS[_dataset->_nb_targets+1][0]);
}

void PartitionScheme::write_optimalpath(string filename, const TOUR& tour){
	ofstream file_OptPath(filename);
	auto itr = tour.begin();
	int tar  = (*itr).first._tarID;
	file_OptPath << tar << '\t' << _points[tar][(*itr).first._subID]._x << '\t' << _points[tar][(*itr).first._subID]._y << '\n';
	itr++;
	for(; itr != tour.end()-1;){
		tar  = (*itr).first._tarID;
		file_OptPath << tar << '\t' << _points[tar][(*itr).first._subID]._x << '\t' << _points[tar][(*itr).first._subID]._y << '\n';
		file_OptPath << tar << '\t' << _points[tar][(*itr).second._subID]._x << '\t' << _points[tar][(*itr).second._subID]._y << '\n';
		itr++;
	}
	tar  = (*itr).first._tarID;
	file_OptPath << _dataset->_nb_targets+1 << '\t' << _points[tar][(*itr).first._subID]._x << '\t' << _points[tar][(*itr).first._subID]._y << '\n';
	file_OptPath.close();
}

void PartitionScheme::print(TOUR& tour){
	// cout << "Tour: ";
...
void PartitionScheme::print_tsp(TOUR& tour){
	// cout << "tsp: ";
	for(auto itr=tour.begin(); itr != tour.end(); itr++){
