...
void STEFormulation::build_model(GRBModel* model_TSP, PartitionScheme* PS, DualFormulation * dl) {
	...
	for(int i = 0; i <_size_var_y; i++) {
		for(int j = 0; j < _size_var_y; j++) {
			_var_y[i][j] = _model->addVar(0.0, 1.0, _partition->_min_risk_tars[i][j], GRB_BINARY, "y_" + itos(i) + "_" + itos(j));
		}
	}
	_model->update();
	...
	*_var_v = _model->addVar(0.0, GRB_INFINITY, 1.0, GRB_CONTINUOUS, "v");
	...
	_model->addConstr(exprA <= exprB, "del_inv_sequence");
	_model->update();
...
pair<double, double> STEFormulation::solve_IP_TSP() {
	try {
		BendersCuts * cb = new BendersCuts(_model, _var_y, _var_v, _partition, _dual);
...
double STEFormulation::add_USER_cuts(double** y_sol) {
	_dual->set_objective(y_sol);
	double obj_dual = _dual->solve();
	GRBLinExpr expr = 0;
	_dual->get_Benders_user_cut(expr, _var_y);
		...
		for (int i = 0; i < num_subtourelim_constraints; i++) {
			...
			for (int j = 0; j < size; j++) {
				for (int k = j + 1; k < size; k++) {
					expr += _var_y[tour[start + j]][tour[start + k]] + y[tour[start + k]][tour[start + j]];
				}
			}
			start += size;
