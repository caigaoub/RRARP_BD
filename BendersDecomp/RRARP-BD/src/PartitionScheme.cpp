...
void PartitionScheme::build(DataHandler& dataset, int nb_dstzn) {
	...
	get_risk_reward_outerTrajc(); // '_min_risk_tars' is generated in this function.
}
	...
	for (s = 0; s < _dataset->_nb_targets; s++) { 
		...
		for (i = 0; i < _nb_dstzn; i++) {
			if (i == 0) { // entry point and exit point are the same
				...
				// 
				if (risk_innerpath[i] > max_risk && risk_innerpath[i] < 1000.0)
					max_risk = risk_innerpath[i];
				if (reward_innerpath[i] > max_reward && reward_innerpath[i] < 1000.0)
					max_reward = reward_innerpath[i];
			}
		}
		// cout << "------ maximum inner risk: " << max_risk << endl;
		// cout << "------ maximum inner reward: " << max_reward << endl;
...
void PartitionScheme::get_risk_reward_outerTrajc() {
	...
	for (s = 1; s <= _dataset->_nb_targets; s++) { // ii) boundary s <-> boudary t
		...
		for (t = 1; t <= _dataset->_nb_targets && s != t; t++) {
	...
	for (s = 1; s <= _dataset->_nb_targets; s++) { 	// iii) boundary s <->  end depot
		idxmat_2 = (s - 1) * 2 * _nb_dstzn + _nb_dstzn + 1;
		val_min_risk = INF;
		for (i = 0; i < _nb_dstzn; i++) {
			val_risk = get_risk_outerTrajc(_points[t][i], _dataset->_depot2_loc);
...
void PartitionScheme::solve_shortestpath(vector<int> & seq, vector<vector<double>> & SDS) {
	...
	idxmat_1 = (idx_circle - 1) * 2 * _nb_dstzn + 1; // index of 1st entry at 1st target visited
	for (int i = 0; i < _nb_dstzn; i++)	{
	...
	idxmat_2 = (idx_circle - 1) * 2 * _nb_dstzn + _nb_dstzn + 1;  // index of 1st exit at 1st target visited
	for (int i = 0; i < _nb_dstzn; i++) {
