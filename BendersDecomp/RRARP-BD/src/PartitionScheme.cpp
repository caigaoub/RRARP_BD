...
void PartitionScheme::get_risk_linearInnerTrajc() {
	/*
...
double PartitionScheme::get_reward_linearInnerTrajc(Vertex entry, Vertex exit, int tar) { 
	...
	for (int j = 0; j < num_intervals; j++) {
		l2 = sqrt(_dataset->_radii[tar] * _dataset->_radii[tar] - l1 *l1);
		d = sqrt(l2 *l2 + pow(len_interval * j, 2));
		double temp = exp(-pow(d - _par_optOBdist[tar], 2) / (2 * pow(_par_varOBdist[tar], 2)));
